...
def generate_table_questions(
        ...
        chapter_number,
        textbook_ref,
        output_folder="output/tb_full"):
    
    """
    Iterates through the tables directory, reads each image and its corresponding text caption,
    and runs them through the model fallback chain to generate Data-Interpretation questions.
    """
    ...
    table_files = [f for f in tables_path.iterdir() if f.suffix.lower() in valid_extensions]
    
    if not table_files:
        print(f"No valid images found in {tables_folder}")
        return None

    print(f"Found {len(table_files)} tables to process for Data Interpretation Questions...")
    
    # Master list to accumulate all generated questions across all images
    all_questions = []

    for idx, img_path in enumerate(table_files, start=1):
        print(f"\n--- Processing Image {idx}/{len(table_files)}: {img_path.name} ---")
        
        # 1. Read corresponding caption file if it exists
        caption_file = tables_path / f"{img_path.stem}_caption.txt"
        caption_text = ""
        ...
        if caption_file.exists():
            ...
            # table_match = re.match(r'^(Fig(?:ure)?\.?\s*\d+\.\d+)', caption_text, re.IGNORECASE)
            # if table_match:
            #     display_name = table_match.group(1) # Extracts "Fig 3.3"
        ...
        data_prompt = f"""
You are an expert CBSE Class 10 assessment designer specializing in data processing.

Analyze the attached image and its original textbook caption:
"Caption: {caption_text}"

Generate at least 3  highly accurate, concept-driven data questions testing table analysis or direct practical deduction.
...
The field "category" MUST strictly and uniquely be "Data-Interpretation".
...
"""
        ...
        while current_model_idx < len(modelChain):
            ...
            try:
                print(f"  Requesting {model_name} for data generation...")
                
                response = client.models.generate_content(
                    model=model_name,
                    contents=[obj_rules,subj_rules,image_part, data_prompt],
        ...
        if topic_result:
            try:
                # Clean clean text variations if any wrapper leaked out
                ...
                print(f"  [Error] Failed parsing JSON string text content for {img_path.name}: {parse_err}")
        else:
            print(f"  [Critical] All models failed to generate content for image: {img_path.name}")
    ...
    if all_questions:
        ...
        filename = os.path.join(
            output_folder,
            f"data_mcqs_ch_{chapter_number}_{safe_chap_name}.json"
        )
